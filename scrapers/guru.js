...
(function runGuruScraper() {
  const TARGET_PATH = '/d/jobs';
  const TIMEOUT_MS = 10_000;
  const CARD_SELECTORS = (typeof GURU_SELECTORS !== 'undefined' && Array.isArray(GURU_SELECTORS.card))
    ? GURU_SELECTORS.card
  ...
  function safeParse(html) {
    try {
      if (typeof parseGuru !== 'function') return [];
      return parseGuru(html);
    } catch {
      return [];
    }
  }
