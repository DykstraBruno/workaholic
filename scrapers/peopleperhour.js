...
(function runPeoplePerHourScraper() {
  const TARGET_PATH = '/freelance-jobs';
  const TIMEOUT_MS = 10_000;
  const CARD_SELECTORS = (typeof PEOPLEPERHOUR_SELECTORS !== 'undefined' && Array.isArray(PEOPLEPERHOUR_SELECTORS.card))
    ? PEOPLEPERHOUR_SELECTORS.card
  ...
  function safeParse(html) {
    try {
      if (typeof parsePeoplePerHour !== 'function') return [];
      return parsePeoplePerHour(html);
    } catch {
      return [];
    }
  }
